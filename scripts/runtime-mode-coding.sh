...
if curl -fsS --max-time 1 "$CONTROL_API_URL/health" >/dev/null 2>&1; then
  ...
  echo "  start: gateway-api memory-api embed-worker llama-server"
  echo "  stop: comfyui media-api media-worker image-worker video-worker 3d-worker rigging-worker animation-worker"
fi
