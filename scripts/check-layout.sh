#!/usr/bin/env bash
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

required_paths=(
  "apps/gateway-api"
  "apps/gateway-api/app/__init__.py"
  "apps/gateway-api/app/main.py"
  "apps/gateway-api/app/config.py"
  "apps/gateway-api/app/media_dashboard.py"
  "apps/gateway-api/app/output_cards.py"
  "apps/gateway-api/app/reference_boards.py"
  "apps/gateway-api/app/clients/__init__.py"
  "apps/gateway-api/app/clients/memory_api.py"
  "apps/gateway-api/app/clients/embed_worker.py"
  "apps/gateway-api/app/clients/model_runtime.py"
  "apps/gateway-api/app/clients/media_api.py"
  "apps/gateway-api/app/clients/prompt_interpreter.py"
  "apps/gateway-api/app/models/__init__.py"
  "apps/gateway-api/app/models/gateway.py"
  "apps/gateway-api/app/services/__init__.py"
  "apps/gateway-api/app/services/chat_proxy.py"
  "apps/gateway-api/app/services/chat_router.py"
  "apps/gateway-api/app/services/media_planner.py"
  "apps/gateway-api/app/services/memory_approval_dashboard.py"
  "apps/gateway-api/app/services/model_mapping.py"
  "apps/gateway-api/app/services/runtime_dashboard.py"
  "apps/gateway-api/app/services/runtime_profile_catalog_defaults.py"
  "apps/gateway-api/app/services/runtime_profile_compatibility_matrix.py"
  "apps/gateway-api/app/services/runtime_profile_dashboard_summary.py"
  "apps/gateway-api/app/services/runtime_profile_operator_checklist.py"
  "apps/gateway-api/app/services/runtime_profile_preflight.py"
  "apps/gateway-api/app/services/runtime_profile_recommendation_summary.py"
  "apps/gateway-api/app/services/runtime_profile_run_catalog.py"
  "apps/gateway-api/app/services/router.py"
  "apps/gateway-api/requirements.txt"
  "apps/gateway-api/Dockerfile"
  "apps/memory-api"
  "apps/memory-api/app/__init__.py"
  "apps/memory-api/app/main.py"
  "apps/memory-api/app/config.py"
  "apps/memory-api/app/clients/__init__.py"
  "apps/memory-api/app/clients/embed_worker.py"
  "apps/memory-api/app/clients/postgres.py"
  "apps/memory-api/app/clients/qdrant.py"
  "apps/memory-api/app/models/__init__.py"
  "apps/memory-api/app/models/memory.py"
  "apps/memory-api/app/services/__init__.py"
  "apps/memory-api/app/services/memory_store.py"
  "apps/memory-api/requirements.txt"
  "apps/memory-api/Dockerfile"
  "apps/memory-api/README.md"
  "apps/embed-worker"
  "apps/embed-worker/app/__init__.py"
  "apps/embed-worker/app/main.py"
  "apps/embed-worker/app/config.py"
  "apps/embed-worker/app/models/__init__.py"
  "apps/embed-worker/app/models/embed.py"
  "apps/embed-worker/app/services/__init__.py"
  "apps/embed-worker/app/services/bge_m3_embedder.py"
  "apps/embed-worker/app/services/embedder_factory.py"
  "apps/embed-worker/app/services/fake_embedder.py"
  "apps/embed-worker/requirements.txt"
  "apps/embed-worker/Dockerfile"
  "apps/embed-worker/README.md"
  "apps/nightly-learning-worker"
  "apps/nightly-learning-worker/app/__init__.py"
  "apps/nightly-learning-worker/app/main.py"
  "apps/nightly-learning-worker/app/config.py"
  "apps/nightly-learning-worker/app/clients.py"
  "apps/nightly-learning-worker/app/report.py"
  "apps/nightly-learning-worker/requirements.txt"
  "apps/nightly-learning-worker/Dockerfile"
  "apps/research-ingestion-worker"
  "apps/research-ingestion-worker/app/__init__.py"
  "apps/research-ingestion-worker/app/main.py"
  "apps/research-ingestion-worker/app/config.py"
  "apps/research-ingestion-worker/app/clients.py"
  "apps/research-ingestion-worker/app/ingestion.py"
  "apps/research-ingestion-worker/app/report.py"
  "apps/research-ingestion-worker/requirements.txt"
  "apps/research-ingestion-worker/Dockerfile"
  "apps/feedback-worker"
  "apps/feedback-worker/app/__init__.py"
  "apps/feedback-worker/app/main.py"
  "apps/feedback-worker/app/config.py"
  "apps/feedback-worker/app/clients.py"
  "apps/feedback-worker/app/gateway_summary.py"
  "apps/feedback-worker/app/improvement.py"
  "apps/feedback-worker/app/improvement_plan.py"
  "apps/feedback-worker/app/improvement_patch_plan.py"
  "apps/feedback-worker/app/learning_loop.py"
  "apps/feedback-worker/app/router_prompt_approval.py"
  "apps/feedback-worker/app/feedback_memory_candidates.py"
  "apps/feedback-worker/app/store.py"
  "apps/feedback-worker/app/report.py"
  "apps/feedback-worker/requirements.txt"
  "apps/feedback-worker/Dockerfile"
  "apps/media-api"
  "apps/media-api/app/__init__.py"
  "apps/media-api/app/main.py"
  "apps/media-api/app/config.py"
  "apps/media-api/app/image.py"
  "apps/media-api/app/jobs.py"
  "apps/media-api/app/store.py"
  "apps/media-api/app/schemas.py"
  "apps/media-api/requirements.txt"
  "apps/media-api/Dockerfile"
  "apps/media-worker"
  "apps/media-worker/app/__init__.py"
  "apps/media-worker/app/main.py"
  "apps/media-worker/app/config.py"
  "apps/media-worker/app/comfyui_client.py"
  "apps/media-worker/app/image_jobs.py"
  "apps/media-worker/app/image_processor.py"
  "apps/media-worker/app/processor.py"
  "apps/media-worker/app/schemas.py"
  "apps/media-worker/requirements.txt"
  "apps/media-worker/Dockerfile"
  "apps/control-api"
  "apps/control-api/app/__init__.py"
  "apps/control-api/app/main.py"
  "apps/control-api/app/config.py"
  "apps/control-api/app/modes.py"
  "apps/control-api/app/status.py"
  "apps/control-api/app/actions.py"
  "apps/control-api/app/schemas.py"
  "apps/control-api/requirements.txt"
  "apps/control-api/Dockerfile"
  "apps/prompt-interpreter-worker"
  "apps/prompt-interpreter-worker/app/__init__.py"
  "apps/prompt-interpreter-worker/app/main.py"
  "apps/prompt-interpreter-worker/app/config.py"
  "apps/prompt-interpreter-worker/app/interpreter.py"
  "apps/prompt-interpreter-worker/app/presets.py"
  "apps/prompt-interpreter-worker/app/schemas.py"
  "apps/prompt-interpreter-worker/app/system_status.py"
  "apps/prompt-interpreter-worker/requirements.txt"
  "apps/prompt-interpreter-worker/Dockerfile"
  "apps/dashboard"
  "apps/dashboard-ui"
  "apps/dashboard-ui/package.json"
  "apps/dashboard-ui/index.html"
  "apps/dashboard-ui/tsconfig.json"
  "apps/dashboard-ui/vite.config.ts"
  "apps/dashboard-ui/Dockerfile"
  "apps/dashboard-ui/README.md"
  "apps/dashboard-ui/src/main.tsx"
  "apps/dashboard-ui/src/App.tsx"
  "apps/dashboard-ui/src/vite-env.d.ts"
  "apps/dashboard-ui/src/api.ts"
  "apps/dashboard-ui/src/types.ts"
  "apps/dashboard-ui/src/theme.ts"
  "apps/dashboard-ui/src/layout/DashboardLayout.tsx"
  "apps/dashboard-ui/src/components/AppHeader.tsx"
  "apps/dashboard-ui/src/components/Sidebar.tsx"
  "apps/dashboard-ui/src/components/RuntimeCards.tsx"
  "apps/dashboard-ui/src/components/RuntimeProfileRecommendationCard.tsx"
  "apps/dashboard-ui/src/components/GpuCard.tsx"
  "apps/dashboard-ui/src/components/LlamaServerCard.tsx"
  "apps/dashboard-ui/src/components/Pc2WorkersCard.tsx"
  "apps/dashboard-ui/src/components/LatestJobCard.tsx"
  "apps/dashboard-ui/src/components/ImageLifecycleCard.tsx"
  "apps/dashboard-ui/src/components/SystemResourceCards.tsx"
  "apps/dashboard-ui/src/components/MemoryApprovalPanel.tsx"
  "apps/dashboard-ui/src/components/SummaryCards.tsx"
  "apps/dashboard-ui/src/components/StatusChip.tsx"
  "apps/dashboard-ui/src/components/StatusCard.tsx"
  "apps/dashboard-ui/src/components/GatesPanel.tsx"
  "apps/dashboard-ui/src/components/ServicesPanel.tsx"
  "apps/dashboard-ui/src/components/ServiceStatusGrid.tsx"
  "apps/dashboard-ui/src/components/LatestImagesPanel.tsx"
  "apps/dashboard-ui/src/components/OutputCards.tsx"
  "apps/dashboard-ui/src/components/ThreeDOutputCards.tsx"
  "apps/dashboard-ui/src/components/ReferenceBoards.tsx"
  "apps/dashboard-ui/src/components/SafeCommandsPanel.tsx"
  "apps/dashboard-ui/src/components/ModeHintsPanel.tsx"
  "apps/dashboard-ui/src/components/WarningsPanel.tsx"
  "apps/dashboard-ui/src/styles.css"
  "apps/media-dashboard"
  "apps/media-dashboard/index.html"
  "apps/media-dashboard/package.json"
  "apps/media-dashboard/README.md"
  "apps/media-dashboard/src/App.tsx"
  "apps/media-dashboard/src/main.tsx"
  "apps/media-dashboard/src/styles.css"
  "configs"
  "configs/environments"
  "configs/environments/README.md"
  "configs/environments/pc1-main.example.yaml"
  "configs/environments/pc1.local.example.yaml"
  "configs/environments/pc2-worker.example.yaml"
  "configs/environments/pc2.local.example.yaml"
  "configs/environments/single-machine.example.yaml"
  "configs/environments/new-machine.template.yaml"
  "configs/continue"
  "configs/continue/config-gateway.yaml.example"
  "configs/continue/config-runtime-direct.yaml.example"
  "configs/feedback.example.yaml"
  "configs/gateway-media.example.yaml"
  "configs/model-registry.example.yaml"
  "configs/media-models.example.yaml"
  "configs/media-api.example.yaml"
  "configs/media-worker.example.yaml"
  "configs/media-workflows.example.yaml"
  "configs/model-routing.yaml"
  "configs/models.yaml"
  "configs/prompt-interpreter.example.yaml"
  "configs/research-sources.example.yaml"
  "configs/runtime-modes.example.yaml"
  "configs/runtime.yaml"
  "packages/shared"
  "packages/schemas"
  "packages/clients"
  "infra/docker"
  "infra/docker/docker-compose.yml"
  "infra/postgres"
  "infra/postgres/init/01-init.sql"
  "infra/qdrant"
  "infra/qdrant/README.md"
  "infra/scripts"
  "deploy/pc1"
  "deploy/pc2"
  "deploy/pc2/README.md"
  "deploy/pc2/docker-compose.worker.example.yml"
  "deploy/pc2/.env.example"
  "docs"
  "docs/index.md"
  "scripts"
  "README.md"
  ".gitignore"
  ".env.example"
  "docs/architecture.md"
  "docs/milestones.md"
  "docs/runtime-rules.md"
  "docs/deployment.md"
  "docs/continue-gateway-config.md"
  "docs/gateway-chat.md"
  "docs/memory-injection.md"
  "docs/feedback.md"
  "docs/feedback-worker.md"
  "docs/feedback-sync.md"
  "docs/learning-loop.md"
  "docs/improvement-plan.md"
  "docs/improvement-patch-planner.md"
  "docs/router-prompt-approval.md"
  "docs/feedback-memory-candidates.md"
  "docs/memory-store-workflow.md"
  "docs/memory-store-audit.md"
  "docs/memory-store-apply-log.md"
  "docs/memory-candidate-approval-helper.md"
  "docs/memory-approval-dry-run-e2e.md"
  "docs/memory-approval-dashboard.md"
  "docs/memory-approval-manual-store-runbook.md"
  "docs/gateway-runtime-switch-runbook.md"
  "docs/codex-prompts.md"
  "docs/docker-foundation.md"
  "docs/memory-api.md"
  "docs/embed-worker.md"
  "docs/model-runtime.md"
  "docs/models.md"
  "docs/runtime-command-pack.md"
  "docs/guided-image-generation.md"
  "docs/dashboard-ui.md"
  "docs/backup-restore.md"
  "docs/environment-profiles.md"
  "docs/gateway-api.md"
  "docs/continue-dev.md"
  "docs/pc2-worker-node.md"
  "docs/research-ingestion.md"
  "docs/feedback-success-memory.md"
  "docs/control-plane.md"
  "docs/prompt-interpreter.md"
  "docs/media-lab.md"
  "docs/image-generation.md"
  "docs/ops"
  "docs/ops/README.md"
  "docs/ops/00-system-map.md"
  "docs/ops/01-fresh-install-pc1.md"
  "docs/ops/02-fresh-install-pc2.md"
  "docs/ops/03-daily-startup.md"
  "docs/ops/04-daily-shutdown.md"
  "docs/ops/05-backup.md"
  "docs/ops/06-restore-new-machine.md"
  "docs/ops/07-troubleshooting.md"
  "docs/ops/08-command-cheatsheet.md"
  "docs/ops/09-git-workflow.md"
  "docs/ops/10-runtime-profile-guide.md"
  "docs/ops/11-first-day-walkthrough.md"
  "docs/ops/12-zero-to-running-checklist.md"
  "docs/ops/13-service-location-reference.md"
  "docs/ops/14-backup-restore-drill.md"
  "docs/ops/15-disaster-recovery-card.md"
  "docs/ops/16-startup-service-matrix.md"
  "docs/ops/17-mode-startup-recipes.md"
  "docs/ops/18-image-mode-entry-checklist.md"
  "docs/ops/19-media-readiness-map.md"
  "docs/ops/20-image-mode-safety-rules.md"
  "docs/ops/21-image-pipeline-entry-plan.md"
  "docs/ops/22-image-processing-pipeline-runbook.md"
  "docs/ops/23-image-model-inventory-guide.md"
  "docs/ops/24-image-first-dry-run-plan.md"
  "docs/ops/25-comfyui-flux-startup-checklist.md"
  "docs/ops/26-comfyui-flux-blockers.md"
  "docs/ops/27-comfyui-flux-startup-evidence-template.md"
  "docs/ops/28-image-mode-vram-safety.md"
  "docs/ops/29-manual-llm-stop-start-plan.md"
  "docs/ops/30-image-mode-return-to-coding.md"
  "docs/ops/31-first-image-dry-run-evidence-review.md"
  "docs/ops/32-first-image-dry-run-evidence-template.md"
  "docs/ops/33-first-image-dry-run-review-checklist.md"
  "docs/ops/34-image-existing-script-map.md"
  "docs/ops/35-first-real-image-generation-drill.md"
  "docs/ops/36-first-real-image-generation-evidence-template.md"
  "docs/ops/37-generated-image-git-safety.md"
  "docs/ops/38-generated-image-output-handling.md"
  "docs/ops/39-first-image-success-record.md"
  "docs/ops/40-image-output-cleanup-policy.md"
  "docs/ops/41-comfyui-workflow-inventory.md"
  "docs/ops/42-flux-schnell-parameter-guide.md"
  "docs/ops/43-comfyui-workflow-change-log.md"
  "docs/ops/44-gateway-real-image-run-drill.md"
  "docs/ops/45-gateway-real-image-evidence-template.md"
  "docs/ops/46-gateway-real-image-troubleshooting.md"
  "docs/ops/47-prompt-variants-plan.md"
  "docs/ops/48-small-batch-image-safety.md"
  "docs/ops/49-image-comparison-notes-template.md"
  "docs/ops/50-batch-output-naming-policy.md"
  "docs/ops/51-media-dashboard-output-review.md"
  "docs/ops/52-media-dashboard-latest-images-schema.md"
  "docs/ops/53-media-dashboard-review-template.md"
  "docs/ops/54-controlled-prompt-variant-generation.md"
  "docs/ops/55-prompt-variant-run-template.md"
  "docs/ops/56-controlled-variant-evidence-template.md"
  "docs/ops/57-prompt-variant-stop-conditions.md"
  "docs/ops/58-prompt-variant-result-review.md"
  "docs/ops/59-git-binary-safety-check.md"
  "docs/ops/60-prompt-quality-improvement-plan.md"
  "docs/ops/61-next-pergola-prompt-set.md"
  "docs/ops/62-negative-prompt-notes.md"
  "docs/ops/63-prompt-quality-review-template.md"
  "docs/ops/64-improved-prompt-run-result-review.md"
  "docs/ops/65-pergola-prompt-lessons-learned.md"
  "docs/ops/66-next-technical-detail-prompt-set.md"
  "docs/ops/67-pergola-project-specific-prompt-pack.md"
  "docs/ops/68-pergola-project-overview-prompts.md"
  "docs/ops/69-pergola-technical-detail-prompts.md"
  "docs/ops/70-pergola-prompt-negative-pack.md"
  "docs/ops/71-pergola-project-prompt-run-plan.md"
  "docs/ops/72-technical-detail-image-run-result-review.md"
  "docs/ops/73-pergola-image-selection-notes.md"
  "docs/ops/74-next-project-specific-prompt-improvements.md"
  "docs/ops/75-pergola-image-reference-board.md"
  "docs/ops/76-pergola-reference-board-review-template.md"
  "docs/ops/77-pergola-usta-briefing-notes.md"
  "docs/ops/78-pergola-reference-board-file-handling.md"
  "docs/ops/79-real-pergola-technical-drawing-prompt-pack.md"
  "docs/ops/80-pergola-measured-view-prompts.md"
  "docs/ops/81-pergola-connection-detail-prompts.md"
  "docs/ops/82-pergola-fastener-and-drilling-prompt-pack.md"
  "docs/ops/83-pergola-technical-drawing-review-template.md"
  "docs/ops/84-technical-drawing-safety-notes.md"
  "docs/ops/85-technical-drawing-run-result-review.md"
  "docs/ops/86-technical-drawing-prompt-lessons.md"
  "docs/ops/87-next-simplified-technical-drawing-prompts.md"
  "docs/ops/88-technical-drawing-selection-notes.md"
  "docs/ops/89-simplified-technical-drawing-run-review.md"
  "docs/ops/90-cad-like-prompt-lessons.md"
  "docs/ops/91-next-cad-like-geometry-only-prompts.md"
  "docs/ops/92-manual-labeling-plan.md"
  "docs/ops/93-geometry-only-cad-run-review.md"
  "docs/ops/94-deterministic-pergola-drawing-plan.md"
  "docs/ops/95-pergola-drawing-geometry-spec.md"
  "docs/ops/96-svg-drawing-safety-and-git-policy.md"
  "docs/ops/97-pergola-drawing-implementation-roadmap.md"
  "docs/ops/98-svg-drawing-tool-skeleton.md"
  "docs/ops/99-pergola-svg-output-review-template.md"
  "docs/ops/100-side-elevation-top-plan-svg.md"
  "docs/ops/101-side-top-svg-review-template.md"
  "docs/ops/104-generic-image-architecture-drawing-roadmap.md"
  "docs/ops/105-pergola-as-case-study.md"
  "docs/ops/106-generic-media-generation-roadmap.md"
  "docs/ops/107-generic-drawing-engine-roadmap.md"
  "docs/ops/108-roadmap-reset-decision-log.md"
  "docs/ops/109-generic-prompt-pack-structure.md"
  "docs/ops/110-prompt-pack-review-template.md"
  "docs/ops/111-generic-image-generation-safety.md"
  "docs/ops/112-generic-drawing-engine-skeleton.md"
  "docs/ops/113-drawing-engine-demo-review-template.md"
  "docs/ops/114-media-dashboard-output-cards-plan.md"
  "docs/ops/115-output-card-api-contract.md"
  "docs/ops/116-output-card-ui-plan.md"
  "docs/ops/117-output-card-source-scanning-policy.md"
  "docs/ops/118-output-card-review-template.md"
  "docs/ops/119-prompt-metadata-capture-plan.md"
  "docs/ops/120-image-metadata-schema.md"
  "docs/ops/121-drawing-metadata-schema.md"
  "docs/ops/122-metadata-sidecar-policy.md"
  "docs/ops/123-output-card-metadata-integration.md"
  "docs/ops/124-metadata-review-template.md"
  "docs/ops/125-output-cards-api-implementation.md"
  "docs/ops/126-output-cards-api-review-template.md"
  "docs/ops/127-dashboard-output-cards-ui.md"
  "docs/ops/128-dashboard-output-cards-ui-review-template.md"
  "docs/ops/129-drawing-metadata-sidecar-implementation.md"
  "docs/ops/130-drawing-metadata-sidecar-review-template.md"
  "docs/ops/131-reference-board-selection-plan.md"
  "docs/ops/132-reference-board-schema.md"
  "docs/ops/133-reference-board-storage-policy.md"
  "docs/ops/134-reference-board-ui-plan.md"
  "docs/ops/135-reference-board-api-plan.md"
  "docs/ops/136-reference-board-review-template.md"
  "docs/ops/137-image-metadata-sidecar-implementation.md"
  "docs/ops/138-image-metadata-sidecar-review-template.md"
  "docs/ops/139-output-card-preview-serving-plan.md"
  "docs/ops/140-preview-serving-api-contract.md"
  "docs/ops/141-preview-serving-security-policy.md"
  "docs/ops/142-preview-serving-ui-plan.md"
  "docs/ops/143-preview-serving-review-template.md"
  "docs/ops/144-output-preview-api-implementation.md"
  "docs/ops/145-output-preview-api-review-template.md"
  "docs/ops/146-dashboard-preview-ui-implementation.md"
  "docs/ops/147-dashboard-preview-ui-review-template.md"
  "docs/ops/148-output-card-metadata-detail-api.md"
  "docs/ops/149-dashboard-metadata-detail-drawer.md"
  "docs/ops/150-output-card-metadata-detail-review-template.md"
  "docs/ops/151-reference-board-safe-runtime-store.md"
  "docs/ops/152-reference-board-store-review-template.md"
  "docs/ops/153-reference-board-api-implementation.md"
  "docs/ops/154-reference-board-api-review-template.md"
  "docs/ops/155-reference-board-item-selection-api.md"
  "docs/ops/156-reference-board-item-selection-review-template.md"
  "docs/ops/157-reference-board-ui-implementation.md"
  "docs/ops/158-reference-board-ui-review-template.md"
  "docs/ops/159-reference-board-ui-cors-card-id-fix.md"
  "docs/ops/160-reference-board-ui-cors-card-id-review-template.md"
  "docs/ops/161-reference-board-detail-view.md"
  "docs/ops/162-reference-board-detail-view-review-template.md"
  "docs/ops/163-reference-board-item-note-edit-api.md"
  "docs/ops/164-reference-board-item-note-edit-review-template.md"
  "docs/ops/165-reference-board-export-plan.md"
  "docs/ops/166-reference-board-export-schema.md"
  "docs/ops/167-reference-board-export-security-policy.md"
  "docs/ops/168-reference-board-export-review-template.md"
  "docs/ops/169-reference-board-json-export-implementation.md"
  "docs/ops/170-reference-board-json-export-review-template.md"
  "docs/ops/171-reference-board-markdown-export-implementation.md"
  "docs/ops/172-reference-board-markdown-export-review-template.md"
  "docs/ops/173-reference-board-export-ui.md"
  "docs/ops/174-reference-board-export-ui-review-template.md"
  "docs/ops/175-reference-board-export-download-plan.md"
  "docs/ops/176-reference-board-download-filename-policy.md"
  "docs/ops/177-reference-board-download-security-policy.md"
  "docs/ops/178-reference-board-download-review-template.md"
  "docs/ops/179-reference-board-markdown-download-implementation.md"
  "docs/ops/180-reference-board-markdown-download-review-template.md"
  "docs/ops/181-reference-board-json-download-implementation.md"
  "docs/ops/182-reference-board-json-download-review-template.md"
  "docs/ops/183-reference-board-download-ui.md"
  "docs/ops/184-reference-board-download-ui-review-template.md"
  "docs/ops/185-reference-board-export-regression-review.md"
  "docs/ops/186-reference-board-export-regression-template.md"
  "docs/ops/187-reference-board-export-polish.md"
  "docs/ops/188-reference-board-export-polish-review-template.md"
  "docs/ops/189-reference-board-workflow-summary.md"
  "docs/ops/190-reference-board-workflow-review-template.md"
  "docs/ops/191-reference-board-hardening-plan.md"
  "docs/ops/192-reference-board-hardening-review-template.md"
  "docs/ops/193-reference-board-error-handling-polish.md"
  "docs/ops/194-reference-board-error-handling-review-template.md"
  "docs/ops/195-reference-board-validation-limits.md"
  "docs/ops/196-reference-board-validation-limits-review-template.md"
  "docs/ops/197-reference-board-malformed-store-regression.md"
  "docs/ops/198-reference-board-malformed-store-regression-review-template.md"
  "docs/ops/199-reference-board-store-repair-plan.md"
  "docs/ops/200-reference-board-store-repair-review-template.md"
  "docs/ops/201-reference-board-store-backup-plan.md"
  "docs/ops/202-reference-board-store-backup-review-template.md"
  "docs/ops/203-reference-board-store-repair-cli-plan.md"
  "docs/ops/204-reference-board-store-repair-cli-review-template.md"
  "docs/ops/205-reference-board-store-validate-cli-implementation.md"
  "docs/ops/206-reference-board-store-validate-cli-review-template.md"
  "docs/ops/207-reference-board-store-backup-cli-implementation.md"
  "docs/ops/208-reference-board-store-backup-cli-review-template.md"
  "docs/ops/209-reference-board-store-repair-cli-implementation.md"
  "docs/ops/210-reference-board-store-repair-cli-review-template.md"
  "docs/ops/211-reference-board-store-repair-regression.md"
  "docs/ops/212-reference-board-store-repair-regression-review-template.md"
  "docs/ops/213-reference-board-duplicate-item-repair-plan.md"
  "docs/ops/214-reference-board-duplicate-item-repair-review-template.md"
  "docs/ops/215-reference-board-stale-item-handling-plan.md"
  "docs/ops/216-reference-board-stale-item-handling-review-template.md"
  "docs/ops/217-reference-board-duplicate-item-repair-implementation.md"
  "docs/ops/218-reference-board-duplicate-item-repair-review-template.md"
  "docs/ops/219-reference-board-duplicate-item-repair-regression.md"
  "docs/ops/220-reference-board-duplicate-item-repair-regression-review-template.md"
  "docs/ops/221-reference-board-stale-item-marking-implementation.md"
  "docs/ops/222-reference-board-stale-item-marking-review-template.md"
  "docs/ops/223-reference-board-stale-item-regression.md"
  "docs/ops/224-reference-board-stale-item-regression-review-template.md"
  "docs/ops/225-reference-board-repair-cli-operator-runbook.md"
  "docs/ops/226-reference-board-repair-cli-operator-runbook-review-template.md"
  "docs/ops/227-reference-board-repair-cli-summary-review.md"
  "docs/ops/228-reference-board-repair-cli-summary-review-template.md"
  "docs/ops/229-reference-board-export-stale-duplicate-status-polish.md"
  "docs/ops/230-reference-board-export-stale-duplicate-status-polish-review-template.md"
  "docs/ops/231-reference-board-backup-retention-plan.md"
  "docs/ops/232-reference-board-backup-retention-review-template.md"
  "docs/ops/233-reference-board-export-review-ui-polish.md"
  "docs/ops/234-reference-board-export-review-ui-polish-review-template.md"
  "docs/ops/235-reference-board-phase-closure-and-m35-roadmap.md"
  "docs/ops/236-reference-board-phase-closure-review-template.md"
  "docs/ops/237-3d-blender-parametric-pipeline-foundation.md"
  "docs/ops/238-3d-blender-parametric-pipeline-foundation-review-template.md"
  "docs/ops/239-generic-parametric-blender-prototype-plan.md"
  "docs/ops/240-generic-parametric-blender-prototype-review-template.md"
  "docs/ops/241-blender-runtime-output-safety-plan.md"
  "docs/ops/242-blender-runtime-output-safety-review-template.md"
  "apps/3d-generator/README.md"
  "apps/3d-generator/generic_parametric_blender.py"
  "scripts/test-3d-generator-skeleton.sh"
  "docs/ops/243-generic-parametric-blender-script-skeleton.md"
  "docs/ops/244-generic-parametric-blender-script-skeleton-review-template.md"
  "configs/3d/generic-object.example.json"
  "scripts/test-3d-parameter-config.sh"
  "docs/ops/245-generic-3d-parameter-config-draft.md"
  "docs/ops/246-generic-3d-parameter-config-review-template.md"
  "scripts/test-3d-dry-run-review.sh"
  "docs/ops/247-first-dry-run-blender-script-review.md"
  "docs/ops/248-first-dry-run-blender-script-review-template.md"
  "docs/ops/249-guarded-first-blender-generation-drill-plan.md"
  "docs/ops/250-guarded-first-blender-generation-drill-review-template.md"
  "docs/ops/251-3d-metadata-sidecar-plan.md"
  "docs/ops/252-3d-metadata-sidecar-review-template.md"
  "docs/ops/253-3d-output-cards-plan.md"
  "docs/ops/254-3d-output-cards-review-template.md"
  "scripts/test-3d-generation-guards.sh"
  "docs/ops/255-guarded-blender-generation-implementation.md"
  "docs/ops/256-guarded-blender-generation-implementation-review-template.md"
  "scripts/test-3d-metadata-sidecar-writer.sh"
  "docs/ops/257-3d-metadata-sidecar-writer.md"
  "docs/ops/258-3d-metadata-sidecar-writer-review-template.md"
  "scripts/test-3d-metadata-sidecar-validator.sh"
  "docs/ops/259-3d-metadata-sidecar-validator.md"
  "docs/ops/260-3d-metadata-sidecar-validator-review-template.md"
  "apps/3d-generator/primitive_builder.py"
  "scripts/test-3d-primitive-builder.sh"
  "docs/ops/261-generic-primitive-builder-core.md"
  "docs/ops/262-generic-primitive-builder-core-review-template.md"
  "apps/3d-generator/blender_adapter.py"
  "scripts/test-3d-blender-adapter.sh"
  "docs/ops/263-blender-adapter-implementation.md"
  "docs/ops/264-blender-adapter-implementation-review-template.md"
  "scripts/3d-first-generation-drill-plan.sh"
  "scripts/test-3d-first-generation-drill-plan.sh"
  "docs/ops/265-first-guarded-local-blender-generation-drill.md"
  "docs/ops/266-first-guarded-local-blender-generation-drill-review-template.md"
  "apps/3d-generator/artifact_verifier.py"
  "scripts/test-3d-artifact-verifier.sh"
  "docs/ops/267-generated-3d-artifact-verification.md"
  "docs/ops/268-generated-3d-artifact-verification-review-template.md"
  "apps/gateway-api/app/media_3d_output_cards.py"
  "apps/media-worker/app/animation_plan_validator.py"
  "apps/media-worker/app/animation_timeline_planner.py"
  "apps/media-worker/app/camera_animation_planner.py"
  "apps/media-worker/app/object_transform_animation_planner.py"
  "apps/media-worker/app/blender_animation_adapter.py"
  "apps/media-worker/app/animation_metadata_sidecar.py"
  "apps/media-worker/app/animation_metadata_validator.py"
  "apps/media-worker/app/animation_preview_renderer.py"
  "configs/animation/animation-metadata.schema.json"
  "configs/animation/animation-metadata.example.json"
  "configs/animation/preview-render-request.schema.json"
  "configs/animation/preview-render-request.example.json"
  "configs/animation/preview-render-operation-plan.example.json"
  "configs/animation/camera-motion.schema.json"
  "configs/animation/camera-orbit.example.yaml"
  "configs/animation/object-motion.schema.json"
  "configs/animation/object-transform.example.yaml"
  "configs/animation/blender-animation-operation-plan.example.json"
  "configs/animation/blender-animation-adapter-request.schema.json"
  "configs/animation/blender-animation-adapter-request.example.json"
  "scripts/test-3d-output-card-api.sh"
  "docs/ops/269-3d-output-card-api.md"
  "docs/ops/270-3d-output-card-api-review-template.md"
  "scripts/test-3d-output-cards-ui.sh"
  "docs/ops/271-dashboard-3d-output-cards-ui.md"
  "docs/ops/272-dashboard-3d-output-cards-ui-review-template.md"
  "scripts/test-3d-reference-board-selection.sh"
  "docs/ops/273-3d-reference-board-selection.md"
  "docs/ops/274-3d-reference-board-selection-review-template.md"
  "scripts/test-m35-phase-closure.sh"
  "docs/ops/275-m35-3d-pipeline-phase-closure.md"
  "docs/ops/276-m35-3d-pipeline-phase-closure-review-template.md"
  "configs/animation/animation-plan.example.yaml"
  "configs/animation/animation-plan.schema.json"
  "scripts/test-animation-pipeline-foundation.sh"
  "scripts/test-animation-plan-schema.sh"
  "scripts/test-animation-plan-validator.sh"
  "scripts/test-animation-timeline-planner.sh"
  "scripts/test-camera-animation-planner.sh"
  "scripts/test-object-transform-animation-planner.sh"
  "scripts/test-blender-animation-adapter-plan.sh"
  "scripts/test-blender-animation-adapter.sh"
  "scripts/test-animation-metadata-sidecar-writer.sh"
  "scripts/test-animation-metadata-validator.sh"
  "scripts/test-preview-render-safety-plan.sh"
  "scripts/test-animation-preview-renderer.sh"
  "scripts/test-animation-artifact-verifier.sh"
  "scripts/test-animation-output-card-api-plan.sh"
  "scripts/test-animation-output-card-api.sh"
  "apps/gateway-api/app/media_animation_output_cards.py"
  "packages/animation-validation/animation_validation/__init__.py"
  "packages/animation-validation/animation_validation/metadata.py"
  "packages/animation-validation/animation_validation/artifacts.py"
  "packages/animation-validation/animation_validation/paths.py"
  "apps/media-worker/app/animation_artifact_verifier.py"
  "configs/animation/animation-output-cards-response.example.json"
  "docs/ops/277-animation-pipeline-foundation.md"
  "docs/ops/278-animation-pipeline-foundation-review-template.md"
  "docs/ops/279-animation-plan-schema.md"
  "docs/ops/280-animation-plan-schema-review-template.md"
  "docs/ops/281-animation-plan-validator.md"
  "docs/ops/282-animation-plan-validator-review-template.md"
  "docs/ops/283-timeline-keyframe-planner-core.md"
  "docs/ops/284-timeline-keyframe-planner-core-review-template.md"
  "docs/ops/285-camera-animation-planner.md"
  "docs/ops/286-camera-animation-planner-review-template.md"
  "docs/ops/287-object-transform-animation-planner.md"
  "docs/ops/288-object-transform-animation-planner-review-template.md"
  "docs/ops/289-blender-animation-adapter-plan.md"
  "docs/ops/290-blender-animation-adapter-plan-review-template.md"
  "docs/ops/291-guarded-blender-animation-implementation.md"
  "docs/ops/292-guarded-blender-animation-implementation-review-template.md"
  "docs/ops/293-animation-metadata-sidecar-writer.md"
  "docs/ops/294-animation-metadata-sidecar-writer-review-template.md"
  "docs/ops/295-animation-metadata-validator.md"
  "docs/ops/296-animation-metadata-validator-review-template.md"
  "docs/ops/297-preview-render-safety-plan.md"
  "docs/ops/298-preview-render-safety-plan-review-template.md"
  "docs/ops/299-guarded-preview-render-implementation.md"
  "docs/ops/300-guarded-preview-render-implementation-review-template.md"
  "docs/ops/301-animation-artifact-verifier.md"
  "docs/ops/302-animation-artifact-verifier-review-template.md"
  "docs/ops/303-animation-output-card-api-plan.md"
  "docs/ops/304-animation-output-card-api-plan-review-template.md"
  "docs/ops/305-animation-output-card-api.md"
  "docs/ops/306-animation-output-card-api-review-template.md"
  "docs/ops/307-dashboard-animation-cards-ui.md"
  "docs/ops/308-dashboard-animation-cards-ui-review-template.md"
  "docs/ops/309-animation-reference-board-selection.md"
  "docs/ops/310-animation-reference-board-selection-review-template.md"
  "docs/ops/311-m36-phase-closure.md"
  "scripts/animation/create-m36-acceptance-scene.py"
  "scripts/run-m36-real-blender-acceptance.sh"
  "scripts/test-animation-reference-board-selection.sh"
  "scripts/test-m36-phase-closure.sh"
  "tools/prompt-packs/README.md"
  "tools/prompt-packs/generic-image/README.md"
  "tools/prompt-packs/generic-image/base-prompts.md"
  "tools/prompt-packs/architecture/README.md"
  "tools/prompt-packs/architecture/exterior-prompts.md"
  "tools/prompt-packs/architecture/interior-prompts.md"
  "tools/prompt-packs/technical-reference/README.md"
  "tools/prompt-packs/technical-reference/visual-reference-prompts.md"
  "tools/drawing-engine/README.md"
  "tools/drawing-engine/generate_demo_svg.py"
  "tools/drawing-engine/drawing_primitives.md"
  "tools/pergola-drawings/generate_pergola_svg.py"
  "tools/pergola-drawings/README.md"
  "scripts/reference-board-malformed-store-regression.sh"
  "scripts/reference-board-store-validate.sh"
  "scripts/reference-board-store-backup.sh"
  "scripts/reference-board-store-repair.sh"
  "scripts/reference-board-store-repair-regression.sh"
  "scripts/reference-board-duplicate-item-repair-regression.sh"
  "scripts/reference-board-stale-item-regression.sh"
  "scripts/image/controlled-variant-plan.sh"
  "scripts/runtime-prepare.sh"
  "scripts/runtime/pc1-sleep-prepare.sh"
  "scripts/runtime/pc1-suspend.sh"
  "scripts/runtime/pc1-startup-coding.sh"
  "scripts/runtime/pc1-startup-media-dry.sh"
  "scripts/runtime/pc1-status.sh"
  "scripts/runtime/pc2-sleep-prepare.sh"
  "scripts/runtime/pc2-suspend.sh"
  "scripts/runtime/pc2-startup-workers.sh"
  "scripts/runtime/pc2-status.sh"
  "scripts/runtime/cluster-sleep-prepare.sh"
  "scripts/runtime/cluster-suspend.sh"
  "scripts/runtime/cluster-startup-coding.sh"
  "scripts/runtime/cluster-startup-media-dry.sh"
  "scripts/runtime/cluster-status.sh"
  "scripts/image/image-readiness.sh"
  "scripts/image/image-dry-run.sh"
  "scripts/image/image-mode-prepare.sh"
  "scripts/image/image-real-run.sh"
  "scripts/image/image-latest.sh"
  "scripts/image/image-safe-shutdown.sh"
  "scripts/image/image-full-cycle.sh"
  "scripts/health.sh"
  "scripts/check-pc2-connectivity.sh"
  "scripts/check-pc2-layout.sh"
  "scripts/pc2-sync-code.sh"
  "scripts/pc2-system-status.sh"
  "scripts/pc2-nightly-worker-up.sh"
  "scripts/pc2-nightly-worker-down.sh"
  "scripts/pc2-nightly-worker-health.sh"
  "scripts/pc2-nightly-worker-dry-run.sh"
  "scripts/pc2-research-worker-up.sh"
  "scripts/pc2-research-worker-down.sh"
  "scripts/pc2-research-worker-health.sh"
  "scripts/pc2-research-worker-dry-run.sh"
  "scripts/pc2-feedback-worker-up.sh"
  "scripts/pc2-feedback-worker-down.sh"
  "scripts/pc2-feedback-worker-health.sh"
  "scripts/pc2-feedback-worker-sample.sh"
  "scripts/pc2-improvement-report.sh"
  "scripts/pc2-prompt-interpreter-up.sh"
  "scripts/pc2-prompt-interpreter-down.sh"
  "scripts/pc2-prompt-interpreter-health.sh"
  "scripts/pc2-prompt-interpreter-sample.sh"
  "scripts/check-python-syntax.sh"
  "scripts/check-media-layout.sh"
  "scripts/check-image-models.sh"
  "scripts/check-comfyui-layout.sh"
  "scripts/check-comfyui-runtime.sh"
  "scripts/install-comfyui-runtime.sh"
  "scripts/download-flux-schnell-models.sh"
  "scripts/check-flux-schnell-models.sh"
  "scripts/comfyui-flux-smoke-test.sh"
  "scripts/comfyui-first-image.sh"
  "scripts/comfyui-vram-status.sh"
  "scripts/plan-flux-schnell-models.sh"
  "scripts/link-comfyui-models.sh"
  "scripts/comfyui-up.sh"
  "scripts/comfyui-down.sh"
  "scripts/comfyui-health.sh"
  "scripts/plan-image-model-downloads.sh"
  "scripts/runtime-status.sh"
  "scripts/runtime-mode-coding.sh"
  "scripts/runtime-mode-image.sh"
  "scripts/runtime-mode-video.sh"
  "scripts/runtime-mode-3d-suite.sh"
  "scripts/runtime-mode-media-off.sh"
  "scripts/check-models.sh"
  "scripts/model-inventory.sh"
  "scripts/model-registry-check.sh"
  "scripts/model-runtime-start.sh"
  "scripts/model-runtime-stop.sh"
  "scripts/model-runtime-status.sh"
  "scripts/model-runtime-health.sh"
  "scripts/model-runtime-switch.sh"
  "scripts/docker-summary-snapshot.sh"
  "scripts/docker-summary-status.sh"
  "scripts/test-gateway-api.sh"
  "scripts/test-gateway-media.sh"
  "scripts/gateway-media-plan.sh"
  "scripts/gateway-media-dry-run.sh"
  "scripts/gateway-media-real-plan.sh"
  "scripts/media-dashboard-status.sh"
  "scripts/media-output-cards-status.sh"
  "scripts/reference-board-store-smoke-test.sh"
  "scripts/media-dashboard-open.sh"
  "scripts/feedback-summary-local.sh"
  "scripts/feedback-sync-status.sh"
  "scripts/feedback-sync-to-pc2.sh"
  "scripts/learning-loop-report-local.sh"
  "scripts/improvement-plan-local.sh"
  "scripts/improvement-patch-plan-local.sh"
  "scripts/router-prompt-approval-local.sh"
  "scripts/feedback-memory-candidates-local.sh"
  "scripts/memory-store-plan-local.sh"
  "scripts/memory-store-approved.sh"
  "scripts/memory-store-real-apply-guardrail.sh"
  "scripts/memory-store-audit-local.sh"
  "scripts/memory-store-apply-log-status.sh"
  "scripts/memory-candidate-approval-helper-local.sh"
  "scripts/memory-candidate-list-local.sh"
  "scripts/memory-approval-dry-run-e2e-local.sh"
  "scripts/memory-approval-dry-run-e2e-status.sh"
  "scripts/memory-store-manual-preflight.sh"
  "scripts/test-media-dashboard.sh"
  "scripts/runtime-dashboard-status.sh"
  "scripts/test-runtime-dashboard.sh"
  "scripts/dashboard-ui-up.sh"
  "scripts/dashboard-ui-down.sh"
  "scripts/dashboard-ui-health.sh"
  "scripts/dashboard-ui-open.sh"
  "scripts/test-dashboard-ui.sh"
  "scripts/test-memory-approval-dashboard.sh"
  "scripts/test-openai-compatible-gateway.sh"
  "scripts/test-gateway-chat-proxy.sh"
  "scripts/test-gateway-chat-router.sh"
  "scripts/test-gateway-memory-injection.sh"
  "scripts/test-gateway-feedback.sh"
  "scripts/test-feedback-worker-bridge.sh"
  "scripts/test-feedback-sync.sh"
  "scripts/test-learning-loop-report.sh"
  "scripts/test-improvement-plan.sh"
  "scripts/test-improvement-patch-plan.sh"
  "scripts/test-router-prompt-approval.sh"
  "scripts/test-feedback-memory-candidates.sh"
  "scripts/test-memory-store-workflow.sh"
  "scripts/test-memory-store-audit.sh"
  "scripts/test-memory-store-apply-log.sh"
  "scripts/test-memory-store-real-apply-guardrail.sh"
  "scripts/test-memory-candidate-approval-helper.sh"
  "scripts/test-memory-approval-dry-run-e2e.sh"
  "scripts/test-memory-store-manual-preflight.sh"
  "scripts/test-continue-gateway.sh"
  "scripts/test-nightly-learning-worker.sh"
  "scripts/test-research-ingestion-worker.sh"
  "scripts/test-feedback-worker.sh"
  "scripts/test-control-api.sh"
  "scripts/test-prompt-interpreter-worker.sh"
  "scripts/test-media-api.sh"
  "scripts/test-media-image-bridge.sh"
  "scripts/test-image-dry-run.sh"
  "scripts/media-image-dry-run.sh"
  "scripts/media-image-real-run.sh"
  "scripts/media-latest-images.sh"
  "scripts/reference-board-export-regression.sh"
  "scripts/test-memory-api.sh"
  "scripts/test-bge-m3-runtime.sh"
  "scripts/test-embed-worker.sh"
  "scripts/test-stack.sh"
)

for path in "${required_paths[@]}"; do
  if [ ! -e "$ROOT/$path" ]; then
    echo "Missing: $path"
    exit 1
  fi
done

for forbidden in \
  "venv" \
  ".venv" \
  "__pycache__" \
  "node_modules" \
  "logs" \
  "data" \
  "runtime" \
  "models" \
  "checkpoints" \
  "moe_memory.db"
do
  if [ -e "$ROOT/$forbidden" ]; then
    echo "Forbidden runtime artifact in codebase: $forbidden"
    exit 1
  fi
done

while IFS= read -r forbidden_path; do
  if [ -n "$forbidden_path" ]; then
    echo "Forbidden generated artifact in codebase: ${forbidden_path#$ROOT/}"
    exit 1
  fi
done < <(find "$ROOT" \
  \( -path "$ROOT/.git" -o -path "$ROOT/.git/*" \) -prune \
  -o \( -name "__pycache__" -o -name "*.pyc" \) -print)

echo "Layout OK"
