...
grep -q -- "- M36.14 Animation Output Card API PLANNED" docs/milestones.md
if rg -n "M36\\.14.*DONE|M37\\.0.*DONE|M38" README.md docs scripts --glob '!scripts/test-animation-preview-renderer.sh' --glob '!scripts/test-animation-output-card-api-plan.sh' >/dev/null; then
