...
if curl -fsS --max-time 1 "$CONTROL_API_URL/health" >/dev/null 2>&1; then
  ...
  echo "  start: comfyui media-api media-worker prompt-interpreter-worker"
  echo "  recommended_stop_for_vram: llama-server"
  echo "  stop: video-worker 3d-worker rigging-worker animation-worker"
fi
