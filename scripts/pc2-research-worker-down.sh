...
echo "Stopping PC-2 Research Ingestion Worker only"
...
ssh -o BatchMode=yes "${PC2_USER}@${PC2_HOST}" \
  "cd '${PC2_SOURCE_ROOT}' && \
   docker compose --env-file '${PC2_ENV_FILE}' -f '${PC2_COMPOSE_FILE}' --profile research stop research-ingestion-worker && \
   docker compose --env-file '${PC2_ENV_FILE}' -f '${PC2_COMPOSE_FILE}' --profile research ps research-ingestion-worker"

echo "PASS: PC-2 Research Ingestion Worker stop command completed"
