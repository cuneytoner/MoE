...
if curl -fsS --max-time 1 "$CONTROL_API_URL/health" >/dev/null 2>&1; then
  ...
  echo "  start: media-api media-worker video-worker prompt-interpreter-worker"
  echo "  stop: llama-server comfyui image-worker 3d-worker rigging-worker animation-worker"
fi
