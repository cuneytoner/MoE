...
if grep -q 'M37.0 .* DONE\|M38.0 .* DONE' "$MILESTONES" "$CODEX_PROMPTS"; then
  echo "future M37/M38 milestones must remain planned" >&2
  exit 1
fi
...
if rg -n "M37\\.0.*DONE|M38\\.0.*DONE" README.md docs scripts --glob '!scripts/test-animation-output-card-api-plan.sh' --glob '!scripts/test-animation-output-card-api.sh' --glob '!scripts/test-animation-preview-renderer.sh' --glob '!scripts/test-dashboard-animation-cards.sh' --glob '!scripts/test-animation-reference-board-selection.sh' --glob '!scripts/test-m36-phase-closure.sh' >/dev/null; then
