...
Worker `up` helpers rebuild their source-built Docker images with `docker compose up -d --build <service>` after sync, so PC-2 does not keep running stale worker code.

Worker HTTP endpoints may not be ready immediately after Docker reports the container as started. The PC-2 health and action helpers retry `/health` before posting dry-run, sample, or improvement requests. On repeated failure, health scripts print Docker Compose service status and recent logs from PC-2.
...
Improvement reports are generated by the feedback worker under `/home/cuneyt/MoE/runtime/reports/improvements` and remain advisory with `apply_supported=false`.
