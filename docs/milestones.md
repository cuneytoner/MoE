...
- Latest completed: M36.11 Guarded Preview Render Implementation.
- Next planned: M36.12 Animation Artifact Verifier.
...
## Milestone 34.52: Reference Board Export Stale/Duplicate Status Polish
...
## Milestone 34.54: Reference Board Export Review UI Polish

Status: DONE

Goals:
- Polish dashboard review display for exported stale and duplicate status.
...
- Define required fields, output file shape, safety flags, config/generator relationships, validation rules, and privacy boundaries.
- Keep the milestone source-only and non-generating.

## Milestone 35.9: 3D Output Cards Plan

Status: DONE

Goals:
- Plan read-only output cards for future generated 3D assets.
- Use metadata sidecars as the discovery and display source.
- Define card shape, metadata summary fields, preview behavior, Gateway/Dashboard boundaries, path safety, and testing plan.
- Avoid generation, runtime mutation, Gateway changes, and Dashboard changes in this milestone.
...
- Reject unsafe paths, malformed metadata, inconsistent summaries, unsafe safety flags, and provenance mismatches.
...
- Keep default behavior plan-only and importable without Blender.
- Render only sampled PNG frames under guarded execution.
