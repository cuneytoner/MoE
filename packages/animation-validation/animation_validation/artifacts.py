#!/usr/bin/env python3
"""Shared read-only animation artifact verifier for M36.12+."""
...
REPO_ROOT = next((path for path in REPO_ROOT_CANDIDATES if (path / "configs" / "animation").is_dir()), SOURCE_REPO_ROOT)
...
def _check_hash(value: object, path: str, issues: list[ArtifactVerificationIssue]) -> None:
    if not isinstance(value, str) or not HASH_RE.fullmatch(value):
        issues.append(_issue("invalid_sha256", path, "value must be 64 lowercase hex characters"))


def _check_safe_id(value: object, path: str, issues: list[ArtifactVerificationIssue]) -> str | None:
...
def _check_preview_relative_directory(value: object, preview_id: str | None, path: str, issues: list[ArtifactVerificationIssue]) -> None:
    if not isinstance(value, str):
...
def validate_preview_render_operation_plan(operation_plan: dict[str, Any]) -> tuple[ArtifactVerificationIssue, ...]:
    ...
    _check_preview_relative_directory(operation_plan.get("relative_output_directory"), operation_plan.get("preview_id"), "$.relative_output_directory", issues)
    ...
    if not isinstance(flags, dict):
        ...
        for key, value in PLAN_ONLY_OPERATION_SAFETY.items():
