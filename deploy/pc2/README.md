...
Worker `up` targets rebuild their service image after source sync by using Docker Compose `up -d --build <service>`. This avoids stale containers after `make pc2-sync-code`.

Worker HTTP readiness can take a few seconds after rebuild/start. Health, dry-run, sample, and improvement helper scripts wait and retry before calling worker endpoints. If readiness fails, the health scripts print Docker Compose service status and recent logs from PC-2.
...
Prompt and routing improvement reports are generated by the same feedback worker:

```bash
make pc2-improvement-report
```

These reports are advisory only and are written under `/home/cuneyt/MoE/runtime/reports/improvements`.
